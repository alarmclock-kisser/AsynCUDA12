...
{
    /// <summary>
    /// Parses delimited text (CSV/TXT) into a host-side <see cref="ClHostTable"/> without touching the GPU.
    ...
    /// </summary>
    public sealed class ClCsvTableLoader
    {
        private readonly ClCsvOptions options;

        /// <summary>
        /// Initializes a new instance of the <see cref="ClCsvTableLoader"/> class.
        /// </summary>
        /// <param name="options">The parsing options; <see cref="ClCsvOptions.Default"/> when <c>null</c>.</param>
        public ClCsvTableLoader(ClCsvOptions? options = null)
        {
            this.options = options ?? ClCsvOptions.Default;
        }
        ...
        /// <returns>The parsed <see cref="ClHostTable"/>.</returns>
        public ClHostTable LoadFile(string path, string? tableName = null)
        {
        ...
        /// <param name="tableName">The resulting table name; the file name (without extension) when <c>null</c>.</param>
        /// <returns>The parsed <see cref="ClHostTable"/>.</returns>
        public ClHostTable LoadFile(string path, IReadOnlyList<ClColumnType> schema, string? tableName = null)
        {
            string text = File.ReadAllText(path, Encoding.UTF8);
            string name = tableName ?? Path.GetFileNameWithoutExtension(path);
        ...
        /// <returns>The parsed <see cref="ClHostTable"/>.</returns>
        public ClHostTable LoadText(string text, string tableName)
        ...
        /// <param name="tableName">The resulting table name.</param>
        /// <returns>The parsed <see cref="ClHostTable"/>.</returns>
        public ClHostTable LoadText(string text, IReadOnlyList<ClColumnType> schema, string tableName)
            => this.Build(text, tableName, schema ?? throw new ArgumentNullException(nameof(schema)));

        // ---- Build pipeline ----

        private ClHostTable Build(string text, string tableName, IReadOnlyList<ClColumnType>? schema)
        {
            ...
            ClHostTable table = new(tableName, rowCount);
            for (int c = 0; c < columnCount; c++)
            {
                string[] cells = ExtractColumn(rows, dataStart, c, columnCount);
                ClColumnType type = schema != null ? schema[c] : InferType(cells, this.options.Culture);
        ...
        private ClHostColumn BuildColumn(string name, ClColumnType type, string[] cells)
        {
            CultureInfo culture = this.options.Culture;
            switch (type)
            {
                case ClColumnType.Int32:
                    return ClHostColumn.Numeric(name, type, ParseInts(cells, culture));
                case ClColumnType.Int64:
                    return ClHostColumn.Numeric(name, type, ParseLongs(cells, culture));
                case ClColumnType.Single:
                    return ClHostColumn.Numeric(name, type, ParseFloats(cells, culture));
                case ClColumnType.Double:
                    return ClHostColumn.Numeric(name, type, ParseDoubles(cells, culture));
                case ClColumnType.Byte:
                    return ClHostColumn.Numeric(name, type, ParseBytes(cells, culture));
                default:
                    return ClHostColumn.Strings(name, cells);
            }
        }
        ...
        private static ClColumnType InferType(string[] cells, CultureInfo culture)
        {
            bool allInt32 = true;
            bool allInt64 = true;
            bool allDouble = true;
            ...
            if (!any) { return ClColumnType.String; }
            if (allInt32) { return ClColumnType.Int32; }
            if (allInt64) { return ClColumnType.Int64; }
            if (allDouble) { return ClColumnType.Double; }
            return ClColumnType.String;
        }
