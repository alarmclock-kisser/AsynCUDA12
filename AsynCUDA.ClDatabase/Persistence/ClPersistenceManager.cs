...
{
    /// <summary>
    /// Reads and writes the on-disk OpenCL database file format ("GPDB"). The format is intentionally
    /// simple, GPU-friendly and column-oriented: numeric columns are stored as contiguous arrays so
    /// they can be loaded straight into device memory, and string columns are stored as a UTF-8 byte buffer
    /// plus integer offsets and lengths. Writes are atomic (temp file + replace).
    /// </summary>
    public sealed class ClPersistenceManager
    {
        ...
        /// <returns>The parsed <see cref="ClHostDatabase"/>.</returns>
        public ClHostDatabase Load(string path)
        {
        ...
        public void Save(string path, ClHostDatabase database)
        {
            ...
            using (BinaryWriter writer = new(stream, Encoding.UTF8, leaveOpen: false))
            {
                ...
                foreach (ClHostTable table in database.Tables)
                {
        ...
        private static ClHostTable ReadTable(BinaryReader reader)
        {
            string name = reader.ReadString();
            int rowCount = reader.ReadInt32();
            int columnCount = reader.ReadInt32();

            ClHostTable table = new(name, rowCount);
        ...
        private static void WriteTable(BinaryWriter writer, ClHostTable table)
        {
            writer.Write(table.Name);
            writer.Write(table.RowCount);
            writer.Write(table.Columns.Count);
            foreach (ClHostColumn column in table.Columns)
            {
        ...
        private static ClHostColumn ReadColumn(BinaryReader reader, int rowCount)
        {
            string name = reader.ReadString();
            ClColumnType type = (ClColumnType) reader.ReadByte();
            ...
            return ClHostColumn.Numeric(name, type, ReadNumericArray(reader, type, elementCount));
        }

        private static void WriteColumn(BinaryWriter writer, ClHostColumn column)
        {
        ...
        private static Array ReadNumericArray(BinaryReader reader, ClColumnType type, int count)
        {
            switch (type)
            {
        ...
        private static void WriteNumericArray(BinaryWriter writer, ClHostColumn column)
        {
            switch (column.Type)
            {
                case ClColumnType.Int32:
        ...
        private static ClHostColumn ReadStringColumn(BinaryReader reader, string name)
        {
            int rowCount = reader.ReadInt32();
            ...
            return ClHostColumn.Strings(name, DecodeStrings(data, offsets, lengths));
        }

        private static void WriteStringColumn(BinaryWriter writer, ClHostColumn column)
        {
