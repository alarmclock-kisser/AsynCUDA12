...
{
    /// <summary>
    /// The comparison/predicate kinds supported by a <see cref="ClQuery"/> filter. These map directly
    ...
    /// <summary>
    /// The aggregate to compute over the rows selected by a <see cref="ClQuery"/>. These map onto the
    /// reduction kernels (count, sum, min/max) and a host-side average derived from sum/count.
    /// </summary>
    public enum ClQueryAggregate
    {
