...
{
    /// <summary>
    /// CPU-only tests for <see cref="PersistenceManager"/>: GPDB save/load roundtrips for every column
    /// type, the UTF-8 string layout (offsets/lengths), atomic overwrite, and malformed-file handling.
    /// These run fully on a GPU-less machine.
    /// </summary>
    [TestClass]
    public sealed class PersistenceManagerTests
    {
        ...
        public void SaveThenLoad_RoundTripsAllColumnTypes()
        {
            ...
            HostDatabase original = TestData.BuildDatabase();

            this.persistence.Save(path, original);
            HostDatabase loaded = this.persistence.Load(path);

            loaded.Tables.Count.ShouldBe(1);
            HostTable table = loaded.Tables[0];
        ...
        {
            string path = TestData.TempFile(".gpdb");
            this.persistence.Save(path, TestData.BuildDatabase());

            HostTable table = this.persistence.Load(path).Tables[0];
        ...
        public void SaveThenLoad_PreservesStrings()
        {
            ...
            table.Columns[6].Type.ShouldBe(GpuColumnType.String);
        ...
        {
            (byte[] data, int[] offsets, int[] lengths) = PersistenceManager.EncodeStrings(new[] { "ab", "", "cde" });
        ...
        public void DecodeStrings_ReconstructsOriginalValues()
        {
            ...
            (byte[] data, int[] offsets, int[] lengths) = PersistenceManager.EncodeStrings(values);

            PersistenceManager.DecodeStrings(data, offsets, lengths).ShouldBe(values);
        }

        [TestMethod]
        public void EncodeStrings_NullEntry_TreatedAsEmpty()
        {
            (byte[] data, int[] offsets, int[] lengths) = PersistenceManager.EncodeStrings(new string[] { null! });
